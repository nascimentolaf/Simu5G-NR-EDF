...
namespace simu5g
{
    ...
    public:
        ...
        inet::Ptr<LteControlInfo> findCtrlInfo(MacCid cid)
        {
            for (auto pkt : sduList_)
            {
                auto info = pkt->getControlInfo();
                if (!info)
                    continue;

                auto lteInfo = dynamic_cast<LteControlInfo *>(info);
                if (!lteInfo)
                    continue;

                if (MacCidToNodeId(cid) == lteInfo->getDestId() && MacCidToLcid(cid) == lteInfo->getLcid())
                {
                    return inet::Ptr<LteControlInfo>(lteInfo);
                }
            }
