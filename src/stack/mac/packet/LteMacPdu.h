...
namespace simu5g
{
    ...
    public:
        ...
        // LteControlInfo *findCtrlInfo(MacCid cid)
        // {
        //     for (cPacketQueue::Iterator iter(*sduList_); !iter.end(); iter++)
        //     {
        //         cPacket *pkt = static_cast<cPacket *>(*iter);
        //         LteControlInfo *info = dynamic_cast<LteControlInfo *>(pkt->getControlInfo());
        //         if (!info)
        //         {
        //             throw cRuntimeError("Packet does not carry LteControlInfo");
        //         }
        //         if (MacCidToNodeId(cid) == info->getDestId() && MacCidToLcid(cid) == info->getLcid())
        //         {
        ...
        LteMacPdu(const LteMacPdu &other) : LteMacPdu_Base(other)
        {
            copy(other);
        }
